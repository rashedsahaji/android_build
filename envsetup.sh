...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^wave_") ; then
        CUSTOM_BUILD=$(echo -n $1 | sed -e 's/^wave_//g')
...
function print_lunch_menu()
{
    ...
    echo ""
    tput bold;
    echo "     Below are all the devices currently available "
    echo "                  to be compiled.     "
    tput sgr0;
    echo ""
...
function lunch()
{
    ...
    if [ "$1" ] ; then
        ...
        tput setaf 2;
        tput bold;
        echo -n "Go ahead and pick a number or enter lunch combo(wave_device-userdebug)... "
        tput sgr0;
        read answer
    fi
